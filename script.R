...
#removing first row ie genre list
genre_mat2 <- as.data.frame(genre_mat1[-1,],stringsAsFactors=FALSE)
for (col in 1:ncol(genre_mat2)) {
  #converting charecters to integers
...
ratingMatrix <- as.matrix(ratingMatrix[,-1])#removing user Ids
...
library(ggplot2)
movie_views <- colCounts(ratingMatrix)#counting views for each movie
table_views <- data.frame(movie = names(movie_views),views = movie_views)#creating dataframe of views
table_views <- table_views[order(table_views$views,decreasing = TRUE), ]#sorting
table_views$title <- NA
for (index in 1:10325) {
  table_views[index,3] <- as.character(subset(movie_data,movie_data$movieId == table_views[index,1])$title)
}
table_views[1:6,]
ggplot(table_views[1:6, ], aes(x = title,y = views))+geom_bar(stat = "identity",fill = "red")+geom_text(aes(label=views),vjust=-0.3, size=3.5)+theme(axis.text.x = element_text(angle = 45,hjust = 1))+ggtitle("Total Views of the Top Flims")
image(ratingMatrix[1:20,1:25], axes = FALSE,main = "Heatmap of the first 25 rows and columns")
...
qplot(average_ratings,fil=I("steelblue"),col=I("red"))+ggtitle("Distribution of the average rating per user")
#Data Normamlization
...
#Building Recommendation System Using R
...
top_recommendations <- 10 #the number of the items to recommend to each user
...
recommendation_matrix <- sapply(predicted_recommendations@items,function(x){
  as.integer(colnames(movie_ratings)[x])})#matrix with the recommendations for each user 
